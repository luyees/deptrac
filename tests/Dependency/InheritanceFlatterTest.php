...
class InheritanceFlatterTest extends TestCase
{
    ...
    public function testFlattenDependencies(): void
    {
        ...
        $astMap->getClassInherits('classWeihnachtsbaum')->willReturn([
            AstInherit::newTraitUse('classBaum', new FileAppearance(new AstFileReference('classWeihnachtsbaum.php'), 3)),
        ]);
        $astMap->getClassInherits('classGeschmückterWeihnachtsbaum')->willReturn([
            AstMap\AstInherit::newExtends('classBaum', new FileAppearance(new AstFileReference('classGeschmückterWeihnachtsbaum.php'), 3))
                ->withPath([
                    AstInherit::newTraitUse('classWeihnachtsbaum', new FileAppearance(new AstFileReference('classBaum.php'), 3)),
                ]),
        ]);
