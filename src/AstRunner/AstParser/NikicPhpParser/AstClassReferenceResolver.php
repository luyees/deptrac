            ...
            if ($node->extends instanceof Node\Name) {
                $this->currentClassReference->addInherit(
                    AstInherit::newExtends(
                        ...
                        new FileAppearance($this->fileReference, $node->extends->getLine())
                    )
                );
            }
            foreach ($node->implements as $implement) {
                $this->currentClassReference->addInherit(
                    AstInherit::newImplements(
                        $implement->toString(),
                        new FileAppearance($this->fileReference, $implement->getLine())
                    )
                );
            }
        }
        ...
        if ($node instanceof Node\Stmt\Interface_) {
            foreach ($node->extends as $extend) {
                $this->currentClassReference->addInherit(
                    AstInherit::newExtends(
                        $extend->toString(),
                        new FileAppearance($this->fileReference, $extend->getLine())
                    )
                );
            }
        }
        ...
        if ($node instanceof Node\Stmt\UseUse) {
            $this->fileReference->addDependency(
                AstDependency::useStmt(
                    $node->name->toString(),
                    new FileAppearance($this->fileReference, $node->name->getLine())
                )
            );
        }
            ...
            foreach ($node->traits as $trait) {
                $this->currentClassReference->addInherit(
                    AstInherit::newTraitUse(
                        $trait->toString(),
                        new FileAppearance($this->fileReference, $trait->getLine())
                    )
                );
            }
        }
        ...
        if ($node instanceof Node\Expr\Instanceof_ && $node->class instanceof Node\Name) {
            $this->currentClassReference->addDependency(
                AstDependency::instanceofExpr(
                    $node->class->toString(),
                    new FileAppearance($this->fileReference, $node->class->getLine())
                )
            );
        }
        ...
        if ($node instanceof Node\Param && $node->type instanceof Node\Name) {
            $this->currentClassReference->addDependency(
                AstDependency::parameter(
                    $node->type->toString(),
                    new FileAppearance($this->fileReference, $node->type->getLine())
                )
            );
        }
            ...
            if ($node->returnType instanceof Node\Name) {
                $this->currentClassReference->addDependency(
                    AstDependency::returnType(
                        ...
                        new FileAppearance($this->fileReference, $node->returnType->getLine())
                    )
                );
            ...
            foreach ($node->types as $type) {
                $this->currentClassReference->addDependency(
                    AstDependency::catchStmt(
                        $type->toString(),
                        new FileAppearance($this->fileReference, $type->getLine())
                    )
                );
            }
        }
